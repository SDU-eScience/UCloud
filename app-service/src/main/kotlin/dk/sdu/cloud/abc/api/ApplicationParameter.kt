...
sealed class ApplicationParameter<V : Any> {
    ...
    fun map(inputParameter: Any?): V {
        return if (inputParameter == null) {
            if (!optional) {
                ...
                defaultValue ?:
                        throw IllegalStateException("Missing default value for optional parameter '$name'")
            }
        } else {
    ...
    data class Integer(
        ...
        override fun internalMap(inputParameter: Any): Int = (inputParameter as? Int) ?: inputParameter.toString().toInt()
