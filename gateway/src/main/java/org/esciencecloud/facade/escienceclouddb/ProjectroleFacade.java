...
public class ProjectroleFacade {
    ...
    public boolean deleteProjectroleById(int id) {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("eScienceCloudJPAPU");
        EntityManager em = emf.createEntityManager();
        em.getTransaction().begin();
        em.remove(em.find(Projectrole.class, id));
        em.getTransaction().commit();
        em.close();
        emf.close();
        return true;
    ...
    public String listAllActiveProjectrole() {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("eScienceCloudJPAPU");
        EntityManager em = emf.createEntityManager();
        List<Projectrole> entityList = new ArrayList<>();
        Query nq1 = em.createNamedQuery("Projectrole.findByActive");
    ...
    public String listAllProjectrole() {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("eScienceCloudJPAPU");
        EntityManager em = emf.createEntityManager();
        List<Projectrole> entityList = new ArrayList<>();
    ...
    public String entityToJson(Projectrole entity) {
        String json = null;
    ...
    public String entityListToJson(List<Projectrole> entityList) {
        String json = null;
        ObjectMapper mapper = new ObjectMapper();
        try {
