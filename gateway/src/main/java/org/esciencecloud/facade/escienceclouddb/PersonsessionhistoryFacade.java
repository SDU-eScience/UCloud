...
public class PersonsessionhistoryFacade {
    ...
    public boolean deletePersonsessionhistoryById(int id) {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("eScienceCloudJPAPU");
        EntityManager em = emf.createEntityManager();
        em.getTransaction().begin();
        em.remove(em.find(Personsessionhistory.class, id));
        em.getTransaction().commit();
        em.close();
        emf.close();
        return true;
    ...
    public String listAllActivePersonsessionhistory() {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("eScienceCloudJPAPU");
        EntityManager em = emf.createEntityManager();
        List<Personsessionhistory> entityList = new ArrayList<>();
        Query nq1 = em.createNamedQuery("Personsessionhistory.findByActive");
    ...
    public String listAllPersonsessionhistory() {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("eScienceCloudJPAPU");
        EntityManager em = emf.createEntityManager();
        List<Personsessionhistory> entityList = new ArrayList<>();
    ...
    public String entityToJson(Personsessionhistory entity) {
        String json = null;
    ...
    public String entityListToJson(List<Personsessionhistory> entityList) {
        String json = null;
        ObjectMapper mapper = new ObjectMapper();
        try {
