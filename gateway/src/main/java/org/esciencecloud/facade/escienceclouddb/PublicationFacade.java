...
public class PublicationFacade {
    ...
    public boolean deletePublicationById(int id) {
        ...
        EntityManager em = emf.createEntityManager();
        em.getTransaction().begin();
        em.remove(em.find(Publication.class, id));
        em.getTransaction().commit();
        em.close();
        emf.close();
        return true;
    ...
    public String listAllActivePublication() {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("eScienceCloudJPAPU");
        EntityManager em = emf.createEntityManager();
        List<Publication> entityList = new ArrayList<>();
        Query nq1 = em.createNamedQuery("Publication.findByActive");
    ...
    public String listAllPublication() {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("eScienceCloudJPAPU");
        EntityManager em = emf.createEntityManager();
        List<Publication> entityList = new ArrayList<>();
    ...
    public String entityToJson(Publication entity) {
        String json = null;
    ...
    public String entityListToJson(List<Publication> entityList) {
        String json = null;
        ObjectMapper mapper = new ObjectMapper();
        try {
