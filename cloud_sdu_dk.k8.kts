config("ceph") { ctx ->
    ...
    if (ctx.enviornment in setOf("test", "development")) {
...
config("storage") { ctx ->
    when (ctx.environment) {
        "development" -> {
            config("mountLocation", "dev")
        }

        "test" -> {
            config("mountLocation", "test")
        }
    }
}
...
config("elasticsearch") { ctx ->
    ...
    when (ctx.environment) {
        "production" -> {
            ...
            configure("clientCount", 2)
            configure("dataCount", 4)
            configure("clientStorage", "5000Gi")
        }
