...
import {
    CmdlineResultFromWorker, DrawEventHandler,
    DrawEventMessage,
    EventStatusFromMain,
    MessageFromWorker,
    MessageKindFromWorker, SharedBufResponseFromWorker,
...
export class VimWasm {
    ...
    start(opts?: StartOptions) {
        ...
        if (this.screen instanceof ScreenCanvas) {
            (this.screen as ScreenCanvas).input.onKeyDownHandler = this.onKeyDownHandler;
        }
        this.running = true;
    ...
    isFreshlyInitialized: boolean = true;

    async reloadBuffer(name: string, content: string): Promise<void> {
        if (!this.isFreshlyInitialized) {
            try {
                await this.cmdline("w!");
            } catch (e) {
            }
        }
        const encoded = new TextEncoder().encode(content);
        await this.dropFile(name, encoded);
    ...
    readBuffer(): Promise<string> {
        ...
        return new Promise(async (resolve, reject) => {
            self.onFileExport = (_, content) => {
                resolve(new TextDecoder().decode(content));
            };

            try {
                await self.cmdline("w!");
            } catch (e) {}
            try {
                await self.cmdline("export %:p");
            } catch (e) {
                reject(e);
            }
        });
    ...
    private async evalJS(path: string, contents: ArrayBuffer) {
        await this.showError(`Shell not available`);
    }

    private async evalFunc(body: string, args: any[], notifyOnly: boolean) {
        return this.worker.notifyEvalFuncError("Shell not available", new Error("Shell not available"), false);
    }
