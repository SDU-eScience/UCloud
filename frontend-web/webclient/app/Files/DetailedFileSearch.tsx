...
import { Flex, Input, Label, InputGroup, OldStamp, Checkbox, Error, OutlineButton, LoadingButton, Icon } from "ui-components";
...
const SearchStamps = ({ stamps, onStampRemove, clearAll }: SearchStampsProps) => (
    <Box pb="5px">
        {[...stamps].map((l) => (<OldStamp onClick={() => onStampRemove(l)} ml="2px" mt="2px" color="blue" key={l}>{l}<Icon size="12" name="close" /></OldStamp>))}
        {stamps.size > 1 ? (<OldStamp ml="2px" mt="2px" color="white" onClick={() => clearAll()}>Clear all<Icon name="close" size={12} /></OldStamp>) : null}
    </Box >
);
