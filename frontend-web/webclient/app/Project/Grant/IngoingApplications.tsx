...
import {ingoingGrantApplications, IngoingGrantApplicationsResponse} from "Project/Grant/index";
...
export const IngoingApplications: React.FunctionComponent = () => {
    ...
    useEffect(() => {
        avatars.updateCache(ingoingApplications.data.items.map(it => it.requestedBy));
    }, [ingoingApplications]);
            ...
            <>
                <Pagination.List
                    ...
                    pageRenderer={() => (
                        <List>
                            {
                                ingoingApplications.data.items.map(app => {
                                    return <ListRow
                                        key={app.id}
                                                            ...
                                                            <Flex flexGrow={1} justifyContent={"flex-end"}>
                                                                {creditFormatter(
                                                                    app.requestedResources.reduce(
                                                                        (prev, curr) => prev + (curr.creditsRequested ?? 0),
                                                                        0
                                                                    ),
                                                                    0
                                                                )}
