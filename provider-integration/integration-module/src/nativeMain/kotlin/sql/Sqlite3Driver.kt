...
class Sqlite3Connection(
    ...
    private val beginStatement = doPrepareStatement("begin", Sqlite3DebugType.BEGIN)
    private val rollbackStatement = doPrepareStatement("rollback", Sqlite3DebugType.ROLLBACK)
    private val commitStatement = doPrepareStatement("commit", Sqlite3DebugType.COMMIT)
    ...
    override fun prepareStatement(statement: String): PreparedStatement {
        return doPrepareStatement(statement, Sqlite3DebugType.QUERY)
    }

    private fun doPrepareStatement(statement: String, type: Sqlite3DebugType): Sqlite3PreparedStatement {
        ...
        try {
            ...
            return Sqlite3PreparedStatement(connPtr, statementPtr, statement, type)
        } catch(ex: Throwable) {
...
class Sqlite3PreparedStatement(
    ...
    private val debugType: Sqlite3DebugType,
) : PreparedStatement {
    ...
    private val parametersForDebugging = HashMap<String, JsonElement>()
    ...
    override suspend fun bindNull(param: String) {
        ...
        parametersForDebugging[param] = JsonNull
    }
    ...
    override suspend fun bindInt(param: String, value: Int) {
        ...
        parametersForDebugging[param] = JsonPrimitive(value)
    }
    ...
    override suspend fun execute(): ResultCursor {
        ...
        if (debug != null) {
            val ctx = DebugContext.create()
            val message = when (debugType) {
                Sqlite3DebugType.QUERY -> DebugMessage.DatabaseQuery(
                    ctx,
                    statementForDebugging,
                    JsonObject(parametersForDebugging)
                )
                Sqlite3DebugType.BEGIN -> DebugMessage.DatabaseTransaction(ctx, DebugMessage.DBTransactionEvent.OPEN)
                Sqlite3DebugType.COMMIT -> DebugMessage.DatabaseTransaction(ctx, DebugMessage.DBTransactionEvent.COMMIT)
                Sqlite3DebugType.ROLLBACK -> DebugMessage.DatabaseTransaction(ctx, DebugMessage.DBTransactionEvent.ROLLBACK)
            }

            if (debugType == Sqlite3DebugType.QUERY) {
                resultCursor.parentDebugContext = ctx
                resultCursor.cursorStart = Time.now()
            }
...
class Sqlite3ResultCursor(
    ...
    var cursorStart = 0L
    var parentDebugContext: DebugContext? = null
    var isCollectingResults = false

    override suspend fun getInt(column: Int): Int? {
    ...
    override suspend fun getLong(column: Int): Long? {
        val colType = sqlite3_column_type(statementPtr.value, column)
        if (colType == SQLITE_NULL) return null
        else if (colType != SQLITE_INTEGER) throw Sqlite3Exception("$column is not a valid integer")
    ...
    override suspend fun getString(column: Int): String? {
        val colType = sqlite3_column_type(statementPtr.value, column)
        if (colType == SQLITE_NULL) return null
    ...
    override suspend fun getBoolean(column: Int): Boolean? {
        val colType = sqlite3_column_type(statementPtr.value, column)
        if (colType == SQLITE_NULL) return null
    ...
    override suspend fun getDouble(column: Int): Double? {
        val colType = sqlite3_column_type(statementPtr.value, column)
        if (colType == SQLITE_NULL) return null
    ...
    override suspend fun next(): Boolean {
        ...
        if (resultCode == SQLITE_DONE) {
            ...
            if (parentDebugContext != null) {
                debugSystem?.sendMessage(
                    DebugMessage.DatabaseResponse(
                        DebugContext.createWithParent(parentDebugContext!!.id),
                        Time.now() - cursorStart,
