        ...
        db.withSession { session ->
            request.items.forEach { license ->
                session.prepareStatement(
                    """
                        insert into ucloud_compute_license_instances (orchestrator_id, server_id)
                        values (:orchestrator_id, :server_id)
                    """
                ).useAndInvokeAndDiscard(
                    prepare = {
                        bindString("orchestrator_id", license.id)
                        bindString("server_id", license.specification.product.id)
                    }
                )
            }
    ...
    suspend fun deleteInstance(request: BulkRequest<License>) {
        db.withSession { session ->
            for (reqItem in request.items) {
                session.prepareStatement(
                    """
                        delete from ucloud_compute_license_instances 
                        where orchestrator_id = :id
                    """
                ).useAndInvokeAndDiscard(
                    prepare = { bindString("id", reqItem.id) }
                )
            }
        }
    }
