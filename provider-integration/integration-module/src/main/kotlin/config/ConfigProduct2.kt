...
data class ConfigProductSchema(
    ...
    fun findCategoryAndProduct(ref: ProductReferenceV2): Pair<Category, IndividualProduct<*>>? {
        val category = compute.find { it.name == ref.category }
            ?: storage.find { it.name == ref.category }
            ?: publicLinks.find { it.name == ref.category }
            ?: publicIps.find { it.name == ref.category }
            ?: licenses.find { it.name == ref.category }
            ?: return null
    ...
    fun findCategory(category: String): Category? {
        return compute.find { it.name == category }
            ?: storage.find { it.name == category }
            ?: publicLinks.find { it.name == category }
            ?: publicIps.find { it.name == category }
            ?: licenses.find { it.name == category }
    }
}
...
data class Category(
    ...
    private lateinit var coreProducts: List<ProductV2>
    ...
    internal fun init() {
        ...
        category = ProductCategory(
            ...
            when (c) {
                ...
                is ProductCost.ResourceQuota -> accUnit(
                    c.selectPrettyUnitOrDefault(type),
                )
            },
            when (c) {
                ...
                is ProductCost.ResourceQuota -> AccountingFrequency.ONCE
            }
        )
...
sealed class ProductCost {
    ...
    data class ResourceQuota(
        ...
        override val type = Type.ResourceQuota
    }
...
fun parse(data: String): ConfigProductSchema {
...
fun parseCost(node: YamlMap): ProductCost {
    ...
    return when (type) {
        ...
        ProductCost.Type.Resource -> {
            val unit = node.getChildNode<YamlNode>("unit")?.asString()
            val interval = node.getChildNode<YamlNode>("accountingInterval")?.asEnum<ProductCost.AccountingInterval>()
            ProductCost.Resource(unit, null, interval)
        }

        ProductCost.Type.ResourceQuota -> {
            val unit = node.getChildNode<YamlNode>("unit")?.asString()
            ProductCost.ResourceQuota(unit)
        }
    }
}

class ProductSubSection(
...
fun parseComputeProducts(
    ...
    val accountingResource = when (cost) {
        ...
        is ProductCost.Resource, is ProductCost.ResourceQuota -> {
            cost as ProductCost.WithUnit
...
fun parseCategory(
    ...
    return Category(categoryName, type, cost, allProducts).also { it.init() }
}
...
fun YamlNode.asAccountingFloat(): Long {
...
inline fun <reified T : Enum<T>> YamlNode.asEnum(): T {
    val stringValue = asString()
...
inline fun <reified T : YamlNode> assertNodeType(node: YamlNode): T {
    return (node as? T) ?: parserError(node, "Invalid type")
}

inline fun <reified T : YamlNode> YamlMap.getChildNode(child: String): T? {
    return entries.entries.find { it.key.content == child }?.value?.let { assertNodeType(it) }
}

fun YamlNode.asString(): String {
    if (this is YamlList) {
...
fun YamlNode.asBoolean(): Boolean {
    if (this is YamlList) {
...
fun parserError(node: YamlNode, message: String): Nothing {
