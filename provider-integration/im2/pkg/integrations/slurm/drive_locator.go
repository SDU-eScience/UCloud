...
func driveIpcServer() {
	retrieveDriveByIdIpc.Handler(func(r *ipc.Request[fnd.FindByStringId]) ipc.Response[orc.Drive] {
		...
		if !controller.BelongsToWorkspace(orc.ResourceOwnerToWalletOwner(drive.Resource), r.Uid) {
			return ipc.Response[orc.Drive]{
				StatusCode: http.StatusNotFound,
				Payload:    orc.Drive{},
			}
	...
	evaluateLocatorsIpc.Handler(func(r *ipc.Request[apm.WalletOwner]) ipc.Response[[]LocatedDrive] {
		if !controller.BelongsToWorkspace(r.Payload, r.Uid) {
...
func EvaluateLocators(owner apm.WalletOwner, category string) []LocatedDrive {
	...
	for categoryName, fs := range ServiceConfig.FileSystems {
		...
		for locatorName, locator := range fs.DriveLocators {
			...
			if locator.Entity != config.SlurmDriveLocatorEntityTypeNone {
				if !ownerInformationOk {
					log.Warn("Unable to resolve owner of drive: %v", owner)
					continue
				}
				if owner.Type == apm.WalletOwnerTypeUser && locator.Entity != config.SlurmDriveLocatorEntityTypeUser {
					continue
				}

				if owner.Type == apm.WalletOwnerTypeProject && locator.Entity != config.SlurmDriveLocatorEntityTypeProject {
					continue
				}
			}
			...
			if locator.Pattern != "" {
				...
				ext := controller.NewScript[map[string]string, scriptResp]()
