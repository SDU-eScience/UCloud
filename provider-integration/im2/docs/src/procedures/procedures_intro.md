...
this, even when not mentioned explicitly. Some examples of epics with design documents include:
...
works as intended. The automatic testing runs using GitHub Actions and creates an entire local cloud system to perform
the tests on.
...
testing.  New issues may arise from testing, which are inserted in the backlog. Depending on the nature of 
...
UCloud depends on various external libraries. [Go](https://go.dev) is used for the backend system.
...
Automatic testing is performed by our Continuous Integration (CI) system. The director grants access to the CI system.
