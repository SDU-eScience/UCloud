package dk.sdu.escience.storage.irods.tools

import dk.sdu.escience.storage.*
import dk.sdu.escience.storage.irods.IRodsConnectionFactory
import dk.sdu.escience.storage.irods.IRodsConnectionInformation
...
class DataTest(
    ...
    private val adminAccount = connectionFactory.createForAccount(adminAccount.first, adminAccount.second)
    ...
    class ScheduledTasksForUser(
        ...
        override fun run() {
            synchronized(lock) {
                _conn = connectionFactory.createForAccount(username, password)
                try {
    ...
    private fun scheduledTaskForUser(username: String, password: String): Runnable {
        ...
        return ScheduledTasksForUser(connectionFactory, username, password)
    }
