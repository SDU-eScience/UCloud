        ...
        return ctx.withSession { session ->
            ...
            val group = session.sendPreparedStatement(
                ...
                """
                    ...
                    from project.groups g left join project.group_members gm on g.id = gm.group_id
                    where
                        id = :group and 
        ...
        return ctx.withSession { session ->
            val results = session.sendPreparedStatement(
                ...
                """
                    ...
                    from project.projects p left join project.groups g on g.project = p.id
                    where
                ...
                .sendPreparedStatement(
                    ...
                    """
                        ...
                        where 
                            ...
                            (:username::text is null or project.check_group_acl(:username, :userIsAdmin, :group))
                    """
                )
                .rows
            ...
            val items = session
                .sendPreparedStatement(
                    ...
                    """
                        select *
                        from project.group_members gm
                        inner join project.groups g on g.id = gm.group_id
                        where
                            (:group = '' or group_id = :group) and
                            (:username::text is null or project.check_group_acl(:username, :userIsAdmin, :group))
                        order by
            ...
            val membersOfAnyGroup = session
                .sendPreparedStatement(
                    ...
                    """
                        ...
                        select m.username
                        from project.project_members m, project.group_members gm join project.groups g on (gm.group_id = g.id)
                        where
            ...
            val projectStatus = session
                .sendPreparedStatement(
                    ...
                    """
                        ...
                        from project.project_members pm, project.projects p
                        where 
                ...
                .sendPreparedStatement(
                    ...
                    """
                        select 
                            ...
                            p.id, 
                            p.title, 
                            project.is_favorite(mem.username, p.id) as is_fav,
                            p.archived,
                            p.parent
                        from 
                            project.project_members mem inner join project.projects p on mem.project_id = p.id
                        where 
                            mem.username = :username and
                            (:showArchived or p.archived = false) and
                            (project.is_favorite(mem.username, p.id))
                ...
                .sendPreparedStatement(
                    ...
                    """
                        ...
                        where 
                            ...
                            (not :noFavorites or not project.is_favorite(mem.username, p.id))
                        order by is_fav desc, p.title
                        offset :offset
                        limit :limit
                    """
        ...
        return ctx.withSession { session ->
            val latestVerification = session
                .sendPreparedStatement(
                    ...
                    """
                        select * 
                        from project.project_membership_verification 
    ...
    suspend fun listIngoingInvites(
        ...
        return ctx.withSession { session ->
            val count = session
                .sendPreparedStatement(
                    { setParameter("username", requestedBy) },
                    "select count(*) from project.invites where username = :username"
                )
                .rows
                .single()
                .getLong(0) ?: 0L
            val items = session
                .sendPreparedStatement(
                    ...
                    """
                        ...
                        from project.invites, project.projects
                        where
    ...
    suspend fun exists(
        ...
        return ctx.withSession { session ->
            session
                .sendPreparedStatement(
                    { setParameter("projectId", projectId) },
                    "select count(*) from project.projects where id = :projectId"
                )
                .rows
                .single()
                .let { it.getLong(0)!! } > 0L
        ...
        return ctx.withSession { session ->
            when (actor) {
                ...
                is Actor.User, is Actor.SystemOnBehalfOfUser -> {
                    if (actor is Actor.User && actor.principal.role in (Roles.PRIVILEGED + Role.PROVIDER)) {
                        // Allowed
                    } else {
            ...
            session
                .sendPreparedStatement(
                    { setParameter("id", id) },
                    "select * from project.projects where id = :id"
                )
                .rows
                .singleOrNull()
                ?.toProject()
        ...
        return ctx.withSession { session ->
            session
                .sendPreparedStatement(
                    ...
                    "select * from project.projects where id IN (SELECT unnest(:ids::text[]))"
                )
                .rows
                .map { it.toProject() }
        }
    ...
    suspend fun lookupPrincipalInvestigator(
        ...
        return ctx.withSession { session ->
            ...
            val pi = session
                .sendPreparedStatement(
                    { setParameter("projectId", projectId) },
                    "select username from project.project_members where project_id = :projectId and role = 'PI' limit 1"
                )
