            ...
            ok(
                FindMetadataResponse(
                    metadataService
                        ...
                        .map {
                            MetadataUpdate(it.path, it.type, it.username, defaultMapper.encodeToString(it.payload))
                        }
                )
            )
        }
        ...
        implement(MetadataDescriptions.updateMetadata) {
            ok(metadataService.updateMetadataBulk(request.updates.map {
                Metadata(
                    ...
                    it.type,
                    it.username,
                    runCatching { defaultMapper.decodeFromString<JsonObject>(it.jsonPayload) }.getOrElse { ex ->
