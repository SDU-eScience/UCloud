...
object LoginCooldownTable : SQLTable("login_cooldown") {
    val username = text("username", notNull = true)
    val expiresAt = timestamp("expiers_at", notNull = true)
    val allowLoginsAfter = timestamp("allow_logind_after", notNull = true)
...
class LoginAttemptAsyncDao(
    ...
    suspend fun logAttempt(db: DBContext, username: String) {
        db.withSession { session ->
            ...
            session.insert(LoginAttemptTable) {
                ...
                set(LoginAttemptTable.createdAt, LocalDateTime.now(DateTimeZone.UTC))
            }
        }
    ...
    suspend fun timeUntilNextAllowedLogin(db: DBContext, username: String): Long? {
        ...
        val recentLoginAttempts = db.withSession { session ->
            val time = max(timeSource() - OBSERVATION_WINDOW, currentCooldown?.allowLoginsAfter ?: -1)
            session
                .sendPreparedStatement(
                    {
                        setParameter("username", username)
                        setParameter("time", LocalDateTime(time, DateTimeZone.UTC).toDate().time / 1000)
                    },
                    """
                        SELECT COUNT(*)
        ...
        if (recentLoginAttempts >= LOCKOUT_THRESHOLD) {
            ...
            val allowLoginsAfter =
                timeSource() + (LOCKOUT_DURATION_BASE_SECONDS.toDouble().pow(newSeverity).toLong())
            ...
            db.withSession { session ->
                ...
                session.insert(LoginCooldownTable) {
                    ...
                    set(LoginCooldownTable.expiresAt, LocalDateTime(allowLoginsAfter + COOLDOWN_EXPIRY))
                    set(LoginCooldownTable.allowLoginsAfter, LocalDateTime(allowLoginsAfter))
                }
            }

            return LOCKOUT_DURATION_BASE_SECONDS.toDouble().pow(newSeverity).toLong()
        }
        ...
        return db.withSession { session ->
            session
                .sendPreparedStatement(
                    {
                        setParameter("username", username)
                        setParameter("expire", timeSource())
                    },
                    """
                        SELECT *
                        FROM login_cooldown
    ...
    companion object : Loggable {
        ...
        const val OBSERVATION_WINDOW =  60 * 5
        const val COOLDOWN_EXPIRY =  60 * 60
