...
object AccountingPlugin : JobManagementPlugin, Loggable {
    ...
    private val lastAttemptedAccountingMapMutex = Mutex()
    private val lastAttemptedAccountingMap = HashMap<String, Long>()
    ...
    private suspend fun JobManagement.account(jobId: String, jobFromServer: VolcanoJob, lastTs: Long, now: Long) {
        lastAttemptedAccountingMapMutex.withLock {
            val lastAttempt = lastAttemptedAccountingMap[jobId] ?: 0L
            if (now - lastAttempt < TIME_BETWEEN_ACCOUNTING) return
            lastAttemptedAccountingMap[jobId] = now
        }

        println("Accounting for $jobId")
        ...
        if (timespent > 0L) {
            ...
            k8.scope.launch {
                try {
                    ...
                    log.info("Caught exception while performing accounting: ${ex.why} ${ex.errorCode} ${ex.httpStatusCode}")
                }
            }
        }
    }
