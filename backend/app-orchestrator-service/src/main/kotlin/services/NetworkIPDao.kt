        ...
        ctx.withSession { session ->
            session.insert(NetworkIPTable) {
                ...
                set(
                    ...
                    defaultMapper.writeValueAsString(network.specification.firewall
        ...
        var networks = rows.map {
            NetworkIP(
                ...
                NetworkIPSpecification(
                    ...
                    firewall = defaultMapper.readValue(it.getField(NetworkIPTable.firewall)),
                ),
                ...
                emptyList(),
                acl = if (flags.includeAcl == true) defaultMapper.readValue(it.getField(NetworkIPTable.acl)) else null
            )
        }
    ...
    suspend fun updateFirewall(
        ...
        return ctx.withSession { session ->
            session
                .sendPreparedStatement(
                    {
                        setParameter("firewall", defaultMapper.writeValueAsString(firewall))
                        setParameter("id", id.id)
                    },
