...
class ProjectDao {
    suspend fun create(ctx: DBContext, id: String, title: String, principalInvestigator: String) {
    ...
    suspend fun delete(ctx: DBContext, id: String) {
        ctx.withSession { session ->
            session
    ...
    suspend fun findById(ctx: DBContext, projectId: String): Project {
        ctx.withSession { session ->
            return session
                .sendPreparedStatement(
    ...
    suspend fun findByIdPrefix(ctx: DBContext, prefix: String): List<String> {
        ctx.withSession { session ->
            return session
                .sendPreparedStatement(
    ...
    suspend fun addMember(ctx: DBContext, projectId: String, member: ProjectMember) {
        ctx.withSession { session ->
            session.insert(ProjectMemberTable) {
    ...
    suspend fun deleteMember(ctx: DBContext, projectId: String, member: String) {
        ctx.withSession { session ->
            session
                .sendPreparedStatement(
                    {
        ...
        ctx.withSession { session ->
            val items = session
                ...
                .map {
                    ...
                    UserProjectSummary(id, title, ProjectMember(user, role), needsVerification, isFavorite)
                }
    ...
    suspend fun verifyMembership(ctx: DBContext, project: String, verifiedBy: String) {
        ctx.withSession { session ->
