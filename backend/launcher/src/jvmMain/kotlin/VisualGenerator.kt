...
fun Graphics2D.drawTextWrapped(
    text: String,
    x: Int,
    y: Int,
    maxWidth: Int
    ...
    wrapText(text, maxWidth).forEach { line ->
        offsetY += (fontMetrics.height * 1.3).toInt()
        drawString(line, x, y + offsetY)
    }
    return offsetY
}

fun Graphics2D.wrapText(
    text: String,
    maxWidth: Int
): List<String> {
    val normalizedText = run {
        val builder = StringBuilder()
        val lines = text.lines()

        var startOfParagraph = true
        for (line in lines) {
            when {
                line.isBlank() -> {
                    builder.appendLine()
    ...
    // TODO(Dan): This code doesn't handle unordered/ordered lists correctly
    ...
    for (word in words) {
        val toAppend = " $word"
        val wordWidth = metrics.stringWidth(toAppend)
        if (currentWidth + wordWidth >= maxWidth) {
            lines.add(builder.toString())
            builder.clear()
            builder.append(word)
            currentWidth = metrics.stringWidth(word)
        } else {
