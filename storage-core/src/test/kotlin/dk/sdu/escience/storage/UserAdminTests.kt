...
abstract class UserAdminTests {
    ...
    abstract val adminConn: Connection
    ...
    fun testCreateAndLogin() {
        ...
        val service = connectionFactory.createForAccount(username, password)
    ...
    fun testModificationInvalidatesPassword() {
        ...
        var service = connectionFactory.createForAccount(username, password)
        service.fileQuery.listAt(service.paths.homeDirectory)
        service.close()
        ...
        try {
            service = connectionFactory.createForAccount(username, password)
            service.fileQuery.listAt(service.paths.homeDirectory)
        } catch (e: Exception) {
